...
class TensorPool:
    """预分配张量池，用于固定维度的forward_chunk优化"""
    
    def __init__(self, device, dtype, chunk_size, hidden_dim):
        self.device = device
        self.dtype = dtype
        ...
        # 预分配常用张量
        self.tmp_masks = torch.ones(1, chunk_size, device=device, dtype=torch.bool).unsqueeze(1)
        self.chunk_masks_template = torch.ones((1, chunk_size, 0), dtype=torch.bool, device=device)
        self.mask_pad_empty = torch.ones((0, 0, 0), dtype=torch.bool, device=device)
        
        # 预分配上采样相关张量
        self.upsample_tmp_masks = torch.ones(1, chunk_size * 2, device=device, dtype=torch.bool).unsqueeze(1)
        self.upsample_chunk_masks_template = torch.ones((1, chunk_size * 2, 0), dtype=torch.bool, device=device)
    
    def get_tmp_masks(self, seq_len):
        if seq_len == self.chunk_size:
            return self.tmp_masks
        else:
            # 回退到动态分配
            return torch.ones(1, seq_len, device=self.device, dtype=torch.bool).unsqueeze(1)
    
    def get_chunk_masks(self, seq_len, offset):
        return torch.ones((1, seq_len, offset), dtype=torch.bool, device=self.device)
    
    def get_upsample_tmp_masks(self, seq_len):
        if seq_len == self.chunk_size * 2:
            return self.upsample_tmp_masks
        else:
            return torch.ones(1, seq_len, device=self.device, dtype=torch.bool).unsqueeze(1)
...
class UpsampleConformerEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        self._fixed_chunk_size = None

    def init_tensor_pool(self, chunk_size: int, device: str, dtype: torch.dtype):
        """初始化张量池用于固定维度优化"""
        print("init_tensor_pool")
        if self._tensor_pool is None or self._fixed_chunk_size != chunk_size:
            self._tensor_pool = TensorPool(device, dtype, chunk_size, self._output_size)
    ...
    def forward_chunk_optimized(
        ...
        """优化版本的forward_chunk，适用于固定维度的输入
        
        使用预分配的张量池来减少动态内存分配，提升性能。
        """
        assert xs.size(0) == 1
        
        # 初始化张量池（如果需要）
        print("init_tensor_pool in forward_chunk_optimized")
        if self._tensor_pool is None or self._fixed_chunk_size != xs.size(1):
            self.init_tensor_pool(xs.size(1), xs.device, xs.dtype)
        
        # 使用预分配的mask
        tmp_masks = self._tensor_pool.get_tmp_masks(xs.size(1))
        ...
        # 处理context
        if context.size(1) != 0:
            context_masks = self._tensor_pool.get_tmp_masks(context.size(1))
            context, _, _ = self.embed(context, context_masks, offset)

        # 预先查找层 + conformer encoder
        xs, pre_lookahead_layer_conv2_cache = self.pre_lookahead_layer(xs, context, pre_lookahead_layer_conv2_cache)
        
        # 使用预分配的chunk_masks和mask_pad
        chunk_masks = self._tensor_pool.get_chunk_masks(xs.size(1), offset)
        mask_pad = self._tensor_pool.mask_pad_empty
        ...
        # 上采样后的embedding处理
        upsample_tmp_masks = self._tensor_pool.get_upsample_tmp_masks(xs.size(1))
        xs, pos_emb, masks = self.up_embed(xs, upsample_tmp_masks, upsample_offset)
        upsample_offset += xs.size(1)

        # conformer encoder for upsampled features
        chunk_masks = self._tensor_pool.get_chunk_masks(xs.size(1), upsample_offset)
        mask_pad = self._tensor_pool.mask_pad_empty
