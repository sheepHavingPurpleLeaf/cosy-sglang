...
class CausalConditionalCFM(ConditionalCFM):
    ...
    @torch.inference_mode()
    def forward(self, mu, mask, n_timesteps, temperature=1.0, spks=None, cond=None, cache={}, finalize=False):
        """Forward diffusion
        ...
        if self.t_scheduler == 'cosine':
            t_span = 1 - torch.cos(t_span * 0.5 * torch.pi)
        mel, cache = self.solve_euler(z, t_span=t_span, mu=mu, mask=mask, spks=spks, cond=cond, cache=cache, finalize=finalize)
        cache['offset'] = offset
        return mel, cache

    def solve_euler(self, x, t_span, mu, mask, spks, cond, cache, finalize):
        """
        Fixed euler solver for ODEs.
        Args:
            x (torch.Tensor): random noise
    ...
    def forward_estimator(self, x, mask, mu, t, spks, cond, cache, finalize):
        if isinstance(self.estimator, torch.nn.Module):
            x, cache1, cache2, cache3, cache4, cache5, cache6, cache7 = self.estimator.forward_chunk(x, mask, mu, t, spks, cond, **cache, finalize=finalize)
            ...
            with self.lock:
                ...
                if finalize:
                    ...
                    # pdb.set_trace()
                    down_blocks_kv_cache_out = cache['down_blocks_kv_cache_out']
                    mid_blocks_kv_cache_out = cache['mid_blocks_kv_cache_out']
                    up_blocks_kv_cache_out = cache['up_blocks_kv_cache_out']
                time_end = time.time()
                # print(f"time_start: {time_start}, time_end: {time_end}, time_cost: {(time_end - time_set_input_shape) * 1000}毫秒")
                assert self.estimator.execute_v2([x.contiguous().data_ptr(),
                                                  mask.contiguous().data_ptr(),
                                                  mu.contiguous().data_ptr(),
                                                  t.contiguous().data_ptr(),
